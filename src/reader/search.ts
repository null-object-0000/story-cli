...
const UNIGRAM_STOP = new Set(
  "的了是在有和与也就都而及或被把让从到于之这那他她它们个不很大人会能去来上下中里时说想要给对我你谁什么怎么为什么因为所以然后还是才又再最一个种些东西事情负责任何从事进行开始成为相关属于涉及通过需要提供得到给予参与实现推动促进加强保证确保帮助支持管理处理操作联系沟通表达描述介绍说明解释回答寻找选择安排准备执行推进开展组织加入退出离开进入到达返回来自影响作用价值方式方法情况问题原因结果过程内容信息知识经验能力需求目标任务工作时间地点人物身份地位角色职业场所环境背景条件机会可能需要应该必须愿意希望认为觉得发现知道了解熟悉记得遗忘忽略关注重视强调突出直".split("")
);

/** 二元组停用字：任一元字命中即丢弃该 bigram（如"的人/是谁/的是/什么/负责/进行"），
 *  避免常见的虚词组合在召回时产生假阳性。内容字（拉/车/饭/做/沉/默/脸/板…）不受影响。 */
const BIGRAM_STOP = new Set(
  "的了吗呢啊吧是这在有和与就也都这那他她它们我你谁什么怎么为因而或及把被让向从到于之等还再又最很太好个叫什负责任何从事进行开始成为相关属于涉及通过需要提供得到给予参与实现推动促进加强保证确保帮助支持管理处理操作联系沟通表达描述介绍说明解释回答寻找选择安排准备执行推进开展组织加入退出离开进入到达返回来自影响作用价值方式方法情况问题原因结果过程内容信息知识经验能力需求目标任务工作时间地点人物身份地位角色职业场所环境背景条件机会可能需要应该必须愿意希望认为觉得发现知道了解熟悉记得遗忘忽略关注重视强调突出直".split("")
);
...
function relationScore(query: string, relation: string, stats: CorpusStats): number {
  ...
  if (big > 0) return Math.min(95, big + uni); // 与锚点同档（95），靠命中内容二元组数量 tie-break 决出
